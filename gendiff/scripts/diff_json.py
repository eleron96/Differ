...
def handle_single_key(indent_str, key, d, is_add, indent_level, indent):
    ...
    if isinstance(d[key], dict):
        ...
        result.extend(compare_dicts(d[key], {},
                                    indent_level + 1, indent) if is_add
                      else compare_dicts({}, d[key], indent_level + 1, indent))
        result.append(f"{indent_str}}}")
    else:
...
def compare_dicts(d1, d2, indent_level, indent):
    ...
    for key in keys:
        if key in d1 and key in d2:
            result.extend(handle_both_keys(indent_str,
                                           key, d1, d2, indent_level, indent))
        elif key in d1:
            result.extend(handle_single_key(indent_str,
                                            key,
                                            d1,
                                            False, indent_level, indent))
        elif key in d2:
            result.extend(handle_single_key(indent_str,
                                            key,
                                            d2,
                                            True, indent_level, indent))
    return result
